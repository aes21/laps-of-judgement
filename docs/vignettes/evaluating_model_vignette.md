...
This vignette highlights the methodology for evaluating the qualifying
lap time prediction model and the simulated posterior draw accuracy of
...
the posterior draw distribution per driver. This is reflective of a peak
performance push lap expected in qualifying, against the average pace of
...
This data is directly compared to the simulated qualification times (5th
percentile) output by the probabilistic model.
...
- `PIT < 0.05`: The driver went faster than the vast majority of
  ...
  setup change, improving track conditions, or a limited number of
  permitted practice laps at initial model input.
...
- `PIT = 0.00`: The observed time falls entirely outside (faster than)
  ...
  substantial sandbagging in practice, model misspecification for that
  driver, or a limited number of permitted practice laps at initial
  model input.
...
can observe the accuracy of the simulation framework against the target
calibration value.

<img src="evaluating_model_vignette_files/figure-gfm/hist-1.png" alt="" width="100%" style="display: block; margin: auto;" />

Most simulation predictions lie within the `0.05` target calibration.
This indicates an accurate prediction, in addition to a model that
generally tends to underestimate (i.e., predict a slower time) peak lap
time pace for this event. One possible explanation for this could be in
the shrinkage effect of partial-pooling (`1 | Team`). For more
information on this, see the [shrinkage
vignette](shrinkage_vignette.md).
